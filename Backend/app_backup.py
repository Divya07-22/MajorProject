# app.py - Complete Backend Application
import os
import json
import pandas as pd
import joblib
...
app = Flask(__name__)
CORS(app, resources={r"/api/*": {"origins": "*"}})  # Configure appropriately for production
...
try:
    ...
    logger.info(f"✅ Connected to blockchain. Account: {signer.address}")
except Exception as e:
...
try:
    ...
    logger.error("Make sure you have run: bash scripts/train_all_models.sh")
    exit(1)

# --- 6. SMART CONTRACTS LOADING ---
try:
    # Load FraudMitigator contract
    ...
    logger.info("✅ FraudMitigator contract loaded")
    
    # Load FraudLedger contract
    ...
    logger.info("✅ FraudLedger contract loaded")
except Exception as e:
    logger.error(f"❌ Smart contract loading failed: {e}")
    logger.error("Make sure contracts are deployed and addresses are in .env")
...
# Transaction Submission and Fraud Check
...
def handle_transaction():
    ...
    try:
        ...
        # Handle both full transaction data and simple amount
        ...
        # 2. Ensemble uses Isolation Forest + XGBoost (no LSTM needed)
        df['lstm_error'] = 0.0  # Placeholder for XGBoost compatibility
        
        # 3. GNN - Placeholder (requires graph structure)
        df['gnn_pred'] = 0
        
        # 4. XGBoost Meta-Model - Final Risk Score
        features_for_xgb = df[required_cols + ['anomaly_score', 'lstm_error', 'gnn_pred']]
        risk_score = float(xgb_model.predict_proba(features_for_xgb)[:, 1][0])
        
        logger.info(f"Risk score calculated: {risk_score:.2%}")
        ...
        # High Risk (>85%)
        if risk_score > 0.85:
            status_message = "High Risk: Account Frozen & Reported to Blockchain"
            user.is_frozen = True
            
            # Initiate voice call verification
            if twilio_client:
                try:
            ...
            if mitigator_contract and web3:
                try:
...
def get_blockchain_logs():
    """Fetch blockchain transaction logs"""
    try:
        current_user_id = int(get_jwt_identity())
        user = db.session.get(User, current_user_id)
        ...
        # Get transactions with blockchain tx_hash
...
def initiate_voice_call():
    """Manually initiate voice call (Admin only)"""
    try:
        claims = get_jwt()
        if claims.get('role') != 'admin':
            return jsonify({"error": "Admin access required"}), 403
...
def voice_response(user_id):
    """Handle Twilio voice call response"""
...
def handle_keypad(user_id):
    """Handle keypad input from voice call"""
