...
# Fill NaN values from LSTM (first 10 rows due to sequence generation)
final_df['lstm_error'].fillna(0, inplace=True)

# Fill any remaining NaN in gnn_pred (should already be 0, but just in case)
final_df['gnn_pred'].fillna(0, inplace=True)


print("Feature set created by combining outputs from Isolation Forest, LSTM, and GNN.")
print(f"Total features: {len(final_df.columns) - 1}")  # -1 for Class column
...
# --- 5. Train the Final XGBoost Classifier ---
...
# --- 7. Save the Final, Most Powerful Model ---
